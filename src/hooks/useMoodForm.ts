...
const validationSchema = Yup.object({
  ...
  text_input: Yup.string()
    .when('using_text', {
      is: true,
      then: schema => schema.required('Masukkan teks input').min(10, 'Teks minimal 10 karakter'),
      otherwise: schema => schema
    }),
  weather: Yup.string().required(),
  healthy: Yup.string().oneOf(['yes', 'no']).required(),
  meal_time: Yup.string().oneOf(['morning', 'noon', 'afternoon', 'night']).required(),
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const response = await fetch('https://ae-automation.fly.dev/webhook/pilih-makanan', {
          method: 'POST',
          headers: {
